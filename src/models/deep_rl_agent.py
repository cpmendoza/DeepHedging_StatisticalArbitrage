...
class DeepAgent(object):
    ...
    def __init__(self, network, state_space, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, cash_constraint, constraint_max, loss_type, lr, dropout_par, isput, prepro_stock, name):
        ...
        elif(self.loss_type == "ENT"):
            self.loss = (1/self.alpha)*tf.math.log(tf.reduce_mean(tf.math.exp(tf.math.scalar_mul(-1,self.alpha)*self.hedging_err)))
    ...
    def loss_out_optim(self, hedging_err, alpha, loss_type):
        if (loss_type == "CVaR"):
            ...
            loss = (1/alpha)*np.log(np.exp(-alpha*hedging_err).mean())
        return loss
...
def train_network(model, network, state_space, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, cash_constraint, constraint_max, loss_type, lr, dropout_par, paths, V_0, V_test, strikes, disc_batch,
dividend_batch, transacion_cost, riskaversion, paths_valid, epochs, display_plot, option, moneyness, isput, prepro_stock):
    ...
    try:
        ...
        if loss_type == "CVaR":
            name = f"{model}_{network}_{state_space}_dropout_{str(int(dropout_par*100))}_{loss_type}_{str(int(riskaversion*100))}_TC_{ str(transacion_cost*100)}_{option}_{moneyness}_{cash_constraint_name}"
        elif loss_type == "ENT":
...
def retrain_network(model, network, state_space, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, cash_constraint, constraint_max, loss_type, lr, dropout_par, paths, V_0, V_test, strikes, disc_batch,
dividend_batch, transacion_cost, riskaversion, paths_valid, epochs, display_plot, option, moneyness, isput, prepro_stock):
...
def network_inference(model, network, state_space, nbs_point_traj, batch_size, nbs_input, nbs_units, nbs_assets, cash_constraint, constraint_max, loss_type, lr, dropout_par, V_test, strikes, disc_batch,
